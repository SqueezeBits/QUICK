...
from quick.awq.utils.calib_data import get_calib_dataset
from quick.awq.quantize.scale import apply_scale, apply_clip
from quick.awq.utils.utils import clear_memory, get_best_device
...
class AwqQuantizer:
    ...
    def quantize(self):
        ...
        if self.version == 'QUICK':
            for i in tqdm(range(len(self.modules)), desc="QUICK: Rope Clearing"):
                # [STEP 5]: Remove RoPE after quantization.
                if isinstance(self.modules[i].self_attn, QuantAttentionFused):
                    del self.modules[i].self_attn.rope
...
class QuantAttentionFused(nn.Module):
    ...
    def forward(self, hidden_states:torch.Tensor, attention_mask=None, *args, **kwargs):
        """No forward - just for model quantization"""
        return None